...
async function loadProductIndex(page) {
  var url =
    `http://localhost:8080/api/product/public/findAll-page?page=` +
    page +
    "&size=" +
    size;
  ...
  for (i = 0; i < list.length; i++) {
    ...
    main += `<div class="col-lg-20p col-md-3 col-sm-6 col-6">
      ...
      list[i].name
                        ...
                        <div class="singlepro">
                            <div class="productsold"><span class="reviewsp">Đã bán: ${
                              list[i].quantitySold
                            }</span></div>
                            <img src="${list[i].imageBanner}" class="imgpro">
                            <span class="proname">${list[i].name}</span>
                            <span class="proprice">${formatmoney(
                              list[i].price
                            )} ${
      list[i].oldPrice == null
        ? ""
        : `<span class="oldprice">${formatmoney(list[i].oldPrice)}</span>`
    }</span>
  ...
  for (i = 1; i <= totalPage; i++) {
    mainpage += `<li onclick="loadProductIndex(${
      Number(i) - 1
    })" class="page-item"><a class="page-link" href="#listsp">${i}</a></li>`;
  }
...
async function loadSanPhamBanChay(page) {
  ...
  var mainpage = "";
  for (i = 1; i <= totalPage; i++) {
    mainpage += `<li onclick="loadSanPhamBanChay(${
      Number(i) - 1
    })" class="page-item"><a class="page-link pointer">${i}</a></li>`;
  }
...
//  console.log(name);
  if (id != null) {
    ...
    document.getElementById("quansale").innerHTML =
      "Đã bán " + result.quantitySold;
    document.getElementById("pricedetail").innerHTML = formatmoney(
      result.price
    );
    result.oldPrice == null
      ? ""
      : (document.getElementById("giacusp").innerHTML = formatmoney(
          result.oldPrice
        ));
    ...
    document.getElementById("masp").innerHTML = result.code;
    document.getElementById("tacgia").innerHTML = result.author;
    document.getElementById("sotrang").innerHTML = result.numPage;
    document.getElementById("nhasx").innerHTML = result.nxb;
    document.getElementById("namsx").innerHTML = result.publishYear;
    document.getElementById("khoiluong").innerHTML = result.weight + "(g)";
    document.getElementById("kichthuoc").innerHTML = result.size;
    document.getElementById("hinhthuc").innerHTML = result.form;
    document.getElementById("danhmuc").innerHTML = result.category.name;
    ...
    var url =
      "http://localhost:8080/api/product/public/search-full?page=0&size=4&sort=id,desc";
    var obj = {
      categoryIds: [result.category.id],
      minPrice: 0,
      maxPrice: 100000000,
    };
    ...
    for (i = 0; i < list.length; i++) {
      ...
      main += `<div class="col-lg-6 col-md-6 col-sm-6 col-6">
                    <a href="detail?id=${list[i].id}&name=${
        list[i].name
      }" class="linkpro">
                        <div class="singlepro">
...
async function clickImgdetail(e) {
  ...
  for (i = 0; i < img.length; i++) {
    document
      .getElementsByClassName("imgldetail")
      [i].classList.remove("imgactive");
  }
...
function clickSize(e, id, price) {
  ...
  for (i = 0; i < size.length; i++) {
    document
      .getElementsByClassName("radio-custom")
      [i].classList.remove("activesize");
  }
...
async function searchFull(page) {
 // lay gia tri query  http;//localhost:8080/products/2?search=C
 // http:host/route/:id(param)?query=..
...
//  );
  ...
  const categoryIds = Array.from(categoryCheckboxes).map(
    (checkbox) => checkbox.value
  );

  var url =
    "http://localhost:8080/api/product/public/search-full?page=" +
    page +
    "&size=" +
    size +
    "&sort=" +
...
//    trademarkIds: trademarkIds,
  ...
  var mainpage = "";
  for (i = 1; i <= totalPage; i++) {
    mainpage += `<li onclick="searchFull(${
      Number(i) - 1
    })" class="page-item"><a class="page-link" class="pointer">${i}</a></li>`;
  }
  document.getElementById("pageable").innerHTML = mainpage;
