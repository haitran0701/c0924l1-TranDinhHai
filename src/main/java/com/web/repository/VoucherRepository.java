...
public interface VoucherRepository extends JpaRepository<Voucher,Long> {
    ...
    @Query("select v from Voucher v where v.minAmount<= ?1 and v.endDate >= current_date and v.startDate <= current_date order by v.discount desc ")
    public List<Voucher> voucherKhaDung(Double amount);
}
